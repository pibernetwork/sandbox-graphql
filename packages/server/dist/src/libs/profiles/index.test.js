...
import { ObjectId } from 'library/src/index.js';
...
test('Query Profile', async () => {
  ...
  `;
    ...
    assert(response.body.kind === 'single');
    expect(response.body.singleResult.errors).toBeUndefined();
    const profile = response.body.singleResult.data?.['profile'];
...
test('Mutation - Add Profile - OK', async () => {
    ...
    profiles.insertOne.mockReturnValue(Promise.resolve({ node: profileMock, errors: [] }));
    const GET_QUERY = gql.default `
...
`;
    ...
    profiles.updateOne.mockReturnValue(Promise.resolve({ node: profileMock, errors: [] }));
    const response = await server.executeOperation({
        query: GET_QUERY,
        variables: {
            _id: '123123123123',
